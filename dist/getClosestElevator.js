...
const getClosestElevator = ({ floorThatWasCalled, elevatorState }) => {
    ...
    const result = availableElevators.reduce((acc, cur) => {
        const accDiff = Math.max(acc.floor, floorThatWasCalled) - Math.min(acc.floor, floorThatWasCalled);
        const curDiff = Math.max(cur.floor, floorThatWasCalled) - Math.min(cur.floor, floorThatWasCalled);
        if (accDiff < curDiff) {
            return acc;
        }
        else
